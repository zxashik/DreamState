...
const MenuItems = ({ isMobile }) => {
  ...
  // const MobileMenuWrapper = ({ children }) => ({ children });

  return menu.map((item) => (
    <li className="flex cursor-pointer text-gray-600 text-sm md:text-lg md:mx-2 leading-3 tracking-normal mt-2 py-2 hover:text-indigo-700 focus:text-indigo-700 focus:outline-none">
      <Link href={item.href}>
        <span
          className={`cursor-pointer ${isMobile ? "mr-20" : "ml-2 font-bold"}`}
        >
          {item.title}
        </span>
      </Link>
